...
def FSDR(df, main_col, other_col, rel_thresh=30, abs_thresh=None,
         return_text=True, markdown=True, value_suffix="",
         comparison_text_larger='larger', comparison_text_smaller='smaller',
         value_format_str='', intro_text="", min_value=None):
    """
    Find Significantly Different Rows (FSDR): finds rows in a pandas DataFrame
    where the values in two columns are significantly different.
    Significantly different in this context means that the values have a
    relative or absolute difference greater than a threshold.
    By default this function returns a chunk of human-readable text
    ...
    """
    # Copy df so any changes we make aren't propagated back to the
    # calling function
    df = df.copy()
    ...
    # (relative to main_col)
    diff = df[main_col] - df[other_col]
    abs_diff = diff.abs()
    perc_diff = (abs_diff / df[main_col]) * 100

    # Calculate rows that are significantly different because the percentage
    # difference is over the relative threshold
    if rel_thresh is not None:
        sig_diff_by_perc = set(perc_diff[perc_diff > rel_thresh].index)
    else:
        sig_diff_by_perc = set()

    # Calculate rows that are significantly different because the absolute
    # difference is over the absolute threshold
    ...
    # Get the subset of the data frame that contains the significantly
    # different rows
    # and sort descending by the values in main_col - so that we get the
    # biggest ones at the top
